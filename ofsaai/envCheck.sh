...
echo "org ver =$ORG_REQUIRED_VERSION"
...
if [[ $ORG_JAVA -eq "11." ]];then
ORG_JAVA="11.0"
fi
ORG_REQUIRED_VERSION=$ORG_JAVA
export ORG_REQUIRED_VERSION
echo "ORG_REQUIRED_VERSION  =  $ORG_REQUIRED_VERSION"
...
if [ $1 = "-s" ];then
JCE=`java -cp .:validateXMLInputs.jar validateXML.checkjce`
else
JCE=`java -cp .:../OFS_AAI/bin/validateXMLInputs.jar validateXML.checkjce`
fi
echo "JCE IS $JCE"
if [ "$JCE" == "false" ];then
 echo "Unlimited cryptographic policy is not enabled in current JDK version. To enable this refer the installation guide for more details" 
 jflag=1
fi
...
if test $myvar = "Linux";then
  ...
  echo "OS Version: $machineversion" 
  if [[ $machineversion == 7* ]]; then
     finalver="7"
	fi
	if [[ $machineversion == 8* ]]; then
     finalver="8"
    fi
	if [[ $machineversion == 24* ]]; then
	 finalver="8"
	fi
	if [[ $machineversion == 9* ]]; then
	 finalver="8"
	fi
...
elif [ $IS_ADW = "TRUE" ]; then
	sqlplus -s /nolog <<-EOF> ${LOGFILE}
	WHENEVER OSERROR EXIT 9;
	WHENEVER SQLERROR EXIT SQL.SQLCODE;
	connect /@$USERNAME $OPTION
	EOF
...
if test $exitStaus == 0;then
	...
	Clientversion_extended=$(echo $clientVersion | cut -c1-4)
	#echo $Clientversion_extended
	export Clientversion_extended
	...
	if [[ $fileversion != *$(echo $clientVersion | cut -c1-4)* ]];then
	   echo " Oracle Client version validation failed.  Current version : $clientVersion. Expected version : lower than $clientVersion  Status : FAIL"
	  dbflag=1
	  else
	  echo " Oracle Client version : $clientVersion. Status : SUCCESS"	  
	  fi
	 ClientVersion=$(echo $clientVersion | cut -c1-4)
	 export ClientVersion
	 echo "client version $ClientVersion"
 if [ $1 != "-s" ] && [ $(echo $OFS_ORA_WAL_ENABLED | tr '[:lower:]' '[:upper:]') = "FALSE" ];then
    ...
    checkDbRetrieveVal () {	
	if [ $(echo $OFS_ORA_WAL_ENABLED | tr '[:lower:]' '[:upper:]') = "TRUE" ] && [ $notsilent -eq 1 ]; then
		res=`sqlplus -s /nolog <<-EOF 
		   WHENEVER OSERROR EXIT 9;
		   WHENEVER SQLERROR EXIT SQL.SQLCODE; 
		   connect /@$SID $OPTION
			   ...
			   if test $# -eq 6;then
			     ...
			     if [ -z "${res}" ];then
				   ...
				   if [[ $5 == *"ORACLE_DB_SERVER"* ]];then					     
					  ...
					  res1=$(echo $res | cut -c1-4)
					  #echo "****************************************"
					  #echo $res
					  #echo $res1
	               fi
				  if [[ "$5" != *${res}* ]];then                               #character value
				      if [[ $2 == *"Oracle Database Partitioning feature is"* ]];then
					    ...
					    if [ $mod = "-s" ];then
				         ...
				         echo "Oracle db version $ORACLE_DB_VERSION"
                         ORACLE_DB_R2=$(echo $res | cut -c1-4)
                         export ORACLE_DB_R2
                         echo "Oracle db R2 version $ORACLE_DB_R2"						 
	...
	versioncheck=$(echo $Clientversion_extended | sed -e 's;\.;0;g')
	#versioncheckserver=$(echo $ORACLE_DB_R2 | sed -e 's;\.;0;g')
	#echo $versioncheck
	#echo $versioncheckserver
	#Checking for Ojdbc8.jar
	if [ $versioncheck -eq 1202 ]; then
	#echo "validated"
	if [[ -f "$ORACLE_HOME/jdbc/lib/ojdbc8.jar" ]]; then
	echo " "
	else
	{
	echo "ERROR: Compatible version of the JDBC driver is not found. Please download & copy the ojdbc8.jar in $ORACLE_HOME/jdbc/lib directory."; 
	dbflag=1; 
	}
	fi
	fi
	...
	if [ $1 != "-s" ];then	 
     checkDbRetrieveVal "select privilege from all_tab_privs_recd where table_name='NLS_INSTANCE_PARAMETERS' and rownum=1" "SELECT privilege is not granted for NLS_INSTANCE_PARAMETERS view." "SELECT privilege is granted for NLS_INSTANCE_PARAMETERS view." "Error while checking select grant for NLS_INSTANCE_PARAMETERS view." "SELECT,READ"
	fi 
	
	#NLS_LENGTH_SEMANTICS
	checkDbRetrieveVal "SELECT  NVL(value , 'BYTE') FROM NLS_INSTANCE_PARAMETERS WHERE PARAMETER='NLS_LENGTH_SEMANTICS'" "Oracle instance must be created with the default NLS_LENGTH_SEMANTICS as BYTE." "NLS_LENGTH_SEMANTICS : BYTE." "Error while fetching NLS_LENGTH_SEMANTICS value." "BYTE"
	...
	#USER_TS_QUOTAS
	if [ $1 != "-s" ];then
	 checkDbRetrieveVal "select privilege from all_tab_privs_recd where table_name='USER_TS_QUOTAS' and rownum=1 " "SELECT privilege is not granted for USER_TS_QUOTAS view." "SELECT privilege is granted for USER_TS_QUOTAS view." "Error while checking select grant for USER_TS_QUOTAS." "SELECT,READ"
	
	 #Checking table space
	 checkDbRetrieveVal "SELECT NVL(SUM(MAX_BYTES),0) FROM USER_TS_QUOTAS where tablespace_name=(Select default_tablespace from user_users)" "The schema must be granted at least 500 MB table space for user : ${USERNAME}." "Schema is granted with at least 500 MB table space for user : ${USERNAME}." "Error while fetching MAX_BYTES from USER_TS_QUOTAS." "500" "Schema is granted with at least 500 MB table space. Current value : Unlimited."	
	fi 
	...
	#Checking Ojdbc.jar
   	if test $ORG_REQUIRED_VERSION == "1.8" -a $ORACLE_DB_VERSION == "12";then
	  [[ ! -f "$ORACLE_HOME/jdbc/lib/ojdbc8.jar" ]] && { echo "ERROR: Compatible version(ojdbc8) of the JDBC driver is not found. Please download & copy the ojdbc8.jar in $ORACLE_HOME/jdbc/lib directory."; dbflag=1; }     
	fi
   else
...
sed '/export OS_VERSION=/d' $HOME/.profile >/dev/null
sed '/export DB_CLIENT_VERSION=/d' $HOME/.profile >/dev/null
echo " " >> $HOME/.profile
echo "export OS_VERSION=\"$OSVERSION\"" >> $HOME/.profile
echo "export DB_CLIENT_VERSION=\"$Clientversion_extended\"" >> $HOME/.profile
. $HOME/.profile
